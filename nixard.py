#!/usr/bin/env python3
"""
nixard-claude.py  –  Nixard v2.0.0
...
Integració TUI: nixard-claude.py.
"""
...
from textual.widgets import Header, Footer, Label, Input, DataTable, RadioSet, RadioButton, LoadingIndicator, Static
...
def get_packages_from_nix_config(attr_path) -> set:
    nix_expr = (
        f'let sys = import <nixpkgs/nixos> {{ configuration = {NIXOS_CONFIG}; }};'
...
def get_packages_from_flake(flake_dir, attr_path) -> set:
    """Evalua un atribut de paquets des d'un flake NixOS."""
    ...
    if rc == 0 and out:
        ...
        if rc2 == 0 and out2:
            try:
                ...
                for drv_info in drv_data.values():
                    out_path = drv_info.get("outputs", {}).get("out", {}).get("path", "")
                    if out_path:
...
def search_cache_candidates(keyword):
    """Cerca candidats a nixpkgs via nix search o search.nixos.org."""
    results = _search_via_nix_cli(keyword)
    if results:
        return results
    return _search_via_json_index(keyword)


def _search_via_nix_cli(keyword):
    out, rc = run_cmd(
        ["nix", "search", "nixpkgs", keyword, "--json",
         "--extra-experimental-features", "nix-command flakes"],
        timeout=30,
    )
...
def _search_via_json_index(keyword, channel="nixos-unstable"):
...
class NixardApp(App):
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns(" ", "Package", "Version", "Status")
        table.cursor_type = "row"
        table.zebra_stripes = False
    ...
    async def load_local_system_data(self):
        ...
        self.query_one("#status-content", Label).update("Local store cached.")
    ...
    async def search_and_show_candidates(self, keyword: str):
        ...
        for pkg in candidates:
            name = pkg["name"]
            is_installed = name in self.local_master_set
    ...
    def update_search_results(self) -> None:
        ...
        for pkg in self.local_master_set:
            if search_value and search_value not in pkg.lower():
                continue
            # Extreiem la versió del nom: "firefox-151.0.1" → "151.0.1"
            m = re.match(r'^.+?-([\d][\d\w\.\-]*)$', pkg)
            version = m.group(1) if m else "[dim]—[/dim]"
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        try:
            ...
            row_data = table.get_row_at(table.cursor_row)
            if row_data:
                self._on_pkg_selected(row_data[1])
        except Exception:
            pass
        ...
        try:
            ...
            if row_data:
                ...
                if pkg_name != self.last_selected_pkg:
                    self._on_pkg_selected(pkg_name)
        except Exception:
            pass

    def _on_pkg_selected(self, pkg_name: str):
        self.last_selected_pkg = pkg_name

        # Panell esquerre: presència per àmbit amb agrupació visual
        ...
        # Panell dret: anàlisi de closure real
    ...
    async def render_nix_deps(self, pkg_name: str, pkg_meta: dict, is_installed: bool):
        ...
        try:
            ...
            if pkg_in_profile and local_version and remote_version and local_version != remote_version:
                status_str = f"[bold]v{local_version}[/bold] [dim](update available: v{remote_version})[/dim]"
            elif pkg_in_profile and local_version:
                status_str = f"[bold]v{local_version}[/bold] [dim](in profile)[/dim]"
            elif local_version:
                status_str = f"[bold]v{local_version}[/bold] [dim](in store, not in profile)[/dim]"
