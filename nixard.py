...
# ──────────────────────────────────────────────
#  DB local (cache de narinfo i cerca de paquets)
# ──────────────────────────────────────────────

NIXARD_DB_PATH = Path.home() / ".local" / "share" / "nixard" / "cache.db"


def _open_db() -> sqlite3.Connection:
    NIXARD_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(str(NIXARD_DB_PATH))
    conn.execute("""
    ...
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS pkg_search (
            channel TEXT,
            attr TEXT,
            version TEXT,
            desc TEXT,
            cached_at REAL,
            PRIMARY KEY (channel, attr)
        )
    """)
    conn.commit()
    return conn
...
def db_get_narinfo(nar_hash: str) -> dict | None:
    try:
        conn = _open_db()
        row = conn.execute(
            "SELECT store_path, file_size, nar_size, refs FROM narinfo WHERE hash=?",
            (nar_hash,)
        ).fetchone()
        conn.close()
        if row:
            refs = row[3].split() if row[3] else []
            return {"store_path": row[0], "file_size": row[1],
                    "nar_size": row[2], "references": refs}
    except Exception:
        pass
    return None


def db_put_narinfo(nar_hash: str, data: dict):
    import time
    try:
        conn = _open_db()
        conn.execute(
            "INSERT OR REPLACE INTO narinfo (hash, store_path, file_size, nar_size, refs, fetched_at)"
            " VALUES (?,?,?,?,?,?)",
            (nar_hash, data["store_path"], data["file_size"], data["nar_size"],
             " ".join(data["references"]), time.time())
        )
        conn.commit()
        conn.close()
    except Exception:
...
def db_has_channel(channel: str) -> bool:
    """Retorna True si la DB té paquets indexats per aquest canal."""
    try:
        conn = _open_db()
        row = conn.execute(
            "SELECT COUNT(*) FROM pkg_search WHERE channel=?", (channel,)
        ).fetchone()
        conn.close()
        return row[0] > 0
    except Exception:
...
def db_get_search(channel: str) -> list | None:
    """Retorna tots els paquets del canal de la DB, o None si no hi ha res indexat."""
    try:
        conn = _open_db()
        rows = conn.execute(
            "SELECT attr, version, desc FROM pkg_search WHERE channel=?",
            (channel,)
        ).fetchall()
        conn.close()
        if rows:
            return [{"attr": r[0], "name": r[0], "version": r[1], "desc": r[2]} for r in rows]
    except Exception:
        pass
    return None


def db_get_channel_age_days(channel: str) -> float | None:
    """Retorna els dies que fa que es va indexar el canal, o None si no hi ha dades."""
    import time
    try:
        conn = _open_db()
        row = conn.execute(
            "SELECT MAX(cached_at) FROM pkg_search WHERE channel=?", (channel,)
        ).fetchone()
        conn.close()
        if row and row[0]:
...
def db_put_search(channel: str, results: list):
    import time
    try:
        conn = _open_db()
        conn.execute("DELETE FROM pkg_search WHERE channel=?", (channel,))
        now = time.time()
        conn.executemany(
            "INSERT OR REPLACE INTO pkg_search (channel, attr, version, desc, cached_at)"
            " VALUES (?,?,?,?,?)",
            [(channel, r["attr"], r.get("version", ""), r.get("desc", ""), now) for r in results]
        )
        conn.commit()
        conn.close()
    except Exception:
...
def db_update_repo(channel: str = None) -> int:
    """
    Actualitza la DB local descarregant el packages.json oficial del canal indicat
    (o el canal del sistema si no s'especifica).
    Usa curl del sistema per descomprimir brotli automàticament.
    Retorna el nombre de paquets descarregats.
    """
    if channel is None:
        channel = detect_system_channel()

    url = f"https://channels.nixos.org/{channel}/packages.json.br"

    # curl --compressed descomprimeix brotli automàticament si el sistema ho suporta;
    # si no, el servidor pot retornar gzip/identity com a fallback.
    result = subprocess.run(
        ["curl", "-sL", "--compressed", "--max-time", "120", url],
        capture_output=True,
    )
    ...
    # Si curl no ha pogut descomprimir (brotli no suportat), intentem python brotli
    if raw[:1] in (b'\xcf', b'\x1b', b'\x1f'):
        # Sembla comprimit: intentem brotli python
        try:
    ...
    all_results = []
    for attr, info in packages.items():
        if not attr:
            continue
        all_results.append({
            "attr": attr,
            ...
            "desc": (info.get("meta") or {}).get("description", ""),
        })

    if all_results:
        db_put_search(channel, all_results)
...
def fetch_narinfo(nar_hash):
    """Consulta la DB local o cache.nixos.org/{hash}.narinfo i retorna metadades."""
    cached = db_get_narinfo(nar_hash)
    ...
    try:
        ...
        result = {"store_path": store_path, "file_size": file_size,
                  "nar_size": nar_size, "references": references}
        db_put_narinfo(nar_hash, result)
        return result
    except Exception:
        return None
...
def search_cache_candidates(keyword):
    """Cerca paquets sempre a la DB local (com ports de FreeBSD).
    Si la DB és buida retorna [] — l'usuari ha de fer 'u' primer.
    Mai fa peticions de xarxa per cercar."""
    channel = detect_system_channel()
    all_pkgs = db_get_search(channel)
    if all_pkgs is None:
        # DB buida: retornem llista buida, la UI avisarà
        return []
    kw = keyword.lower()
    return [r for r in all_pkgs if kw in r["attr"].lower() or kw in r.get("desc", "").lower()]
...
def _search_via_json_index(keyword, channel=None):
    """Cerca TOTS els resultats a search.nixos.org paginant automàticament.
    Si hi ha dades a la DB local, les usa directament sense fer peticions de xarxa."""
    if channel is None:
        channel = detect_system_channel()

    # Consulta la DB local primer
    cached = db_get_search(channel)
    if cached:
        kw = keyword.lower()
        return [r for r in cached if kw in r["attr"].lower()]

    all_results = []
    page_size = 50
    offset = 0
    max_results = 2000  # Límit de seguretat
    while offset < max_results:
        url = (f"https://search.nixos.org/api/search"
               f"?channel={channel}&query={keyword}&type=packages"
...
class ExportModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="export-dialog"):
            ...
            yield Button("[[ Exit ]]", id="export-exit-btn", variant="default")
    ...
    def action_export(self):
        name = self.query_one("#export-name-input", Input).value.strip()
        if not name:
            return
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "export-exit-btn":
            event.stop()
            self._do_exit()
...
class SudoPasswordModal(ModalScreen):
    ...
    #sudo-hint { color: #aaaaaa; margin-top: 1; }
    #sudo-confirm-btn {
        margin-top: 1;
        width: 100%;
    }
...
class SaveNixardModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="save-dialog"):
            ...
            yield Label("Enter → save  (leave blank to discard)", id="save-hint")
...
class NixardApp(App):
    """Nixard v3.0.0 – local package database + closure analysis."""
    ...
    def __init__(self):
        ...
        self._version_map = {}
        self._local_master_base_set = set()
    ...
    async def load_local_system_data(self):
        ...
        # Construeix el conjunt de noms base (sense versió) per a matching
        self._local_master_base_set = set()
        for pkg in self.local_master_set:
            base = re.sub(r'-[\d][\d\w\.\-]*$', '', pkg)
            self._local_master_base_set.add(base if base else pkg)
        ...
        await self._update_db_status_bar(detected_channel)
        self.update_search_results()
        self.query_one("#packages-table", DataTable).focus()

    async def _update_db_status_bar(self, channel: str = None):
        """Actualitza el sub_title del Header amb l'estat de la DB."""
        from datetime import datetime
        import time
        if channel is None:
            channel = await asyncio.to_thread(detect_system_channel)
        db_age = await asyncio.to_thread(db_get_channel_age_days, channel)
        if db_age is None:
            self._db_status_str = "Repo DB empty — press u to index"
        else:
            days = int(db_age)
            hours = int((db_age * 24) % 24)
            last_str = datetime.fromtimestamp(time.time() - db_age * 86400).strftime("%Y-%m-%d %H:%M")
            age_str = f"{days}d {hours}h" if days > 0 else f"{hours}h"
            warn = " ⚠ update recommended" if db_age > 7 else ""
            self._db_status_str = f"Repo DB: {last_str} ({age_str}){warn}"
    ...
    def action_update_repo(self):
        """Tecla u: actualitza la DB local amb tots els paquets del canal del sistema."""
        self.run_worker(self._do_update_repo(), exclusive=False)

    async def _do_update_repo(self):
        channel = await asyncio.to_thread(detect_system_channel)
        self.notify(f"Updating repo DB for {channel}…", severity="information")
        self.query_one("#status-content", Label).update(
            f"[dim]Updating repo DB for {channel}…[/dim]"
        )
        count = await asyncio.to_thread(db_update_repo, channel)
        if count > 0:
            self.notify(f"Repo DB updated: {count} packages ({channel})", severity="information")
            self.query_one("#status-content", Label).update(
                f"[bold]Repo DB updated[/bold]\n"
                f"[dim]{count} packages cached\nChannel: {channel}[/dim]"
            )
            await self._update_db_status_bar(channel)
        else:
            self.notify("Repo DB update failed or no packages found.", severity="warning")
            self.query_one("#status-content", Label).update("[dim]Repo DB update failed.[/dim]")
    ...
    def action_toggle_mark(self):
        ...
        mark = "[bold white]\u25cf[/bold white]" if pkg_name in self.marked_packages else " "
        try:
            ...
            self.render_table_rows(preserve_cursor=True)
    ...
    def action_export(self):
        ...
        # Recollim metadades dels paquets marcats (cerquem a TOTS els resultats, no només la pàgina actual)
        marked_list = []
        for pkg_name in sorted(self.marked_packages):
            pkg_meta = None
            for row in self._all_search_results:
                if row[0] == pkg_name and len(row) > 3 and row[3]:
                    pkg_meta = row[3]
                    break
            if not pkg_meta:
    ...
    def action_nix_edit(self):
        ...
        # Construim la llista de paquets marcats amb metadades (cerquem a TOTS els resultats)
        marked_list = []
        for pkg_name in sorted(self.marked_packages):
            pkg_meta = None
            for row in self._all_search_results:
    ...
    def _show_restored_packages(self, packages: list, export_name: str, versions: dict = None):
        ...
        for pkg in sorted(packages):
            # Netejar nom base (treure versió si la porta)
            base_name = re.sub(r'-[\d][\d\w\.\-]*$', '', pkg)
            display_name = base_name if base_name and len(base_name) > 2 else pkg
            in_local = (display_name in self._local_master_base_set or
                        pkg in self.local_master_set or
                        display_name in self.local_master_set)
            ...
            restored_list.append((display_name, version, status, None))

        self._all_search_results = restored_list
        self.current_results = list(self._all_search_results)
    ...
    async def search_and_show_candidates(self, keyword: str):
        channel = await asyncio.to_thread(detect_system_channel)
        db_ready = await asyncio.to_thread(db_has_channel, channel)

        if not db_ready:
            self.query_one("#status-content", Label).update(
                "[bold]Repo DB empty.[/bold]\n\n"
                "Press [bold]u[/bold] to index\n"
                "packages for channel\n"
                f"[dim]{channel}[/dim]"
            )
            self._all_search_results = []
            self.current_results = []
        ...
        candidates = await asyncio.to_thread(search_cache_candidates, keyword)

        # Índex de noms base dels paquets instal·lats
        ...
        for pkg in candidates:
            name = pkg["name"]  # search results ja són noms nets (attr)
        ...
        self._all_search_results = sorted(combined_list, key=lambda x: x[0].lower())
        self.current_results = list(self._all_search_results)
        self.render_table_rows()
        self.query_one("#status-content", Label).update(
            f"{len(self._all_search_results)} candidates found.\n"
            f"[dim](local DB · {channel})[/dim]"
        )

    def update_search_results(self) -> None:
        search_value = self.query_one("#search-box", Input).value.lower()
        seen_base = {}  # base_name → (base_name, version, status, None)
        for pkg in self.local_master_set:
            if search_value and search_value not in pkg.lower():
                continue
            # Separem nom base i versió
            m_ver = re.search(r'-([\d][\d\w\.\-]*)$', pkg)
            if m_ver:
                base_name = pkg[:m_ver.start()]
                version = m_ver.group(1)
            else:
                base_name = pkg
                version = self._version_map.get(pkg) or "[dim]—[/dim]"
            # Deduplicar: guardem el primer (o el que té versió més informativa)
            if base_name not in seen_base:
                seen_base[base_name] = (base_name, version, "[bold]installed[/bold]", None)
        self._all_search_results = sorted(seen_base.values(), key=lambda x: x[0].lower())
        self.current_results = list(self._all_search_results)
        self.render_table_rows()

    def render_table_rows(self, preserve_cursor: bool = False):
        table = self.query_one("#packages-table", DataTable)
        ...
        # Restaurar posició del cursor sense scroll forçat
        if preserve_cursor and saved_row and saved_row < len(self._all_search_results):
            table.move_cursor(row=saved_row, animate=False)
        else:
            table.move_cursor(row=0, animate=False)
        self._update_result_count()

    # ── Indicador de resultats ──

    def _update_result_count(self):
        """Mostra el nombre total de resultats al sub_title."""
        total = len(self._all_search_results)
        mark_count = len(self.marked_packages)
        parts = []
        if self._db_status_str:
            parts.append(self._db_status_str)
        if mark_count > 0:
            parts.append(f"{mark_count} package{'s' if mark_count != 1 else ''} marked  ·  press e to export")
        if total > 0:
    ...
    def _on_pkg_highlighted(self, pkg_name: str):
        ...
        def _in_scope(name, scope_pkgs):
            """Comprova si un paquet és a un scope per nom exacte o nom base."""
            if name in scope_pkgs:
                return True
            if base_name != name and base_name in scope_pkgs:
                return True
            # Comprova si qualsevol entrada del scope coincideix per nom base
            for p in scope_pkgs:
                p_base = re.sub(r'-[\d][\d\w\.\-]*$', '', p)
                if p_base == name or p_base == base_name or p == name or p == base_name:
                    return True
            return False
    ...
    def _on_pkg_closure(self, pkg_name: str):
        ...
        base_name = re.sub(r'-[\d][\d\w\.\-]*$', '', pkg_name)
        found_locally = any(
            pkg_name in pkgs or base_name in pkgs or
            any(re.sub(r'-[\d][\d\w\.\-]*$', '', p) == pkg_name for p in pkgs)
            for pkgs in self.packages_by_scope.values()
        )
